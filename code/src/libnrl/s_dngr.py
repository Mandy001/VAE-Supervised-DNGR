...
class SDNGR(object):
    ...
    def train(self):
        ...
        labeled_datas = tf.nn.embedding_lookup(last_encoder_layer, labeled_indice)
        ...
        mean_loss_dngr = tf.reduce_mean(loss)

        #dngr的loss + 分类的loss
        mean_loss = mean_loss_dngr * 0.3 + mean_clf_loss * 1

        optimizer = tf.train.RMSPropOptimizer(learning_rate=2e-3).minimize(mean_loss)
        ...
        with tf.Session() as sess:
            ...
            for i in range(400):
                ...
                if i % 10 == 0:
                    loss_val, rec_val, clf_val = sess.run([mean_loss, mean_loss_dngr , mean_clf_loss], feed_dict=feed_dict)
                    self.losses.append(loss_val)
                    print("step = {}\tloss = {}\tndgr_loss = {}\tclf_loss = {}".format(i, loss_val, rec_val, clf_val))
